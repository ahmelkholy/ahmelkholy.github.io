...
class UltimateBibTeXProcessor:
    ...
    def get_venue_info(self, entry):
        ...
        if entry_type == "article":
            ...
            venue = entry.get("publisher", entry.get("journal", entry.get("howpublished", "")))
        else:
            # Try multiple fields as fallback
            venue = entry.get("journal", entry.get("booktitle", entry.get("institution", 
...
venue: '{venue}'"""
        ...
        if abstract:
            ...
            description = descriptions.get(entry_type, "This publication contributes to the academic literature.")
    ...
    def parse_bibtex_entry(self, entry_text):
        ...
        while pos < len(field_content):
            ...
            if field_content[field_start] == '{':
                ...
                if next_field:
                    field_value = field_content[field_start:field_start + next_field.start()].strip()
                else:
    ...
    def process_files(self, bibtex_files):
        ...
        for entry in all_entries:
            try:
                ...
                print(f"Error creating file for entry {entry_key} ({entry_title[:30]}...): {e}")
        ...
        print(f"- Total publications now: {len(list(self.publications_dir.glob('*.md')))}")
...
def main():
    # Define paths
    base_dir = Path("d:/PROJECTs-site/ahmelkholy.github.io")
    publications_dir = base_dir / "_publications"

    bibtex_files = [
        publications_dir / "Exported Items.bib",
        publications_dir / "googleschlral.bib",
    ]
